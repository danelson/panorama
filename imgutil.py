...
def gaussian(sigma, order=0, radius=0, norm=True):
    ...
    xrange_ = numpy.arange(-radius, radius + 1)
    denom = 1 / (2 * (sigma ** 2))
    data = numpy.exp(-denom * (xrange_ ** 2))

    # derivatives of Gaussians are products of polynomials (Hermite polynomials)
    # from Front-End Vision, pg. 54
    if order == 1:
        data = -data * (xrange_ / (sigma ** 2))
    elif order == 2:
        data *= (xrange_ ** 2 - sigma ** 2) / (sigma ** 4)
    elif order == 3:
        data = -data * ((xrange_ ** 3 - 3 * xrange_ * sigma ** 2) / (sigma ** 6))
    elif order == 4:
        data *= (xrange_ ** 4 - 6 * xrange_ ** 2 * sigma ** 2 + 3 * sigma ** 4) / (sigma ** 8)
...
def image_show(image, title="image", norm=True):
